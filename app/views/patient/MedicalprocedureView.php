...
class MedicalprocedureView
{
    ...
    public function show(): void
    {
        ...
        <head>
            ...
            <link rel="stylesheet" href="assets/css/components/consultation-modal.css?v=<?= time() ?>">
            <link rel="stylesheet" href="assets/css/layout/aside/aside.css">
            <link rel="icon" type="image/svg+xml" href="assets/img/logo.svg">
            <style>
                /* Force modal width and grid layout to fix narrow display issues */
                #add-consultation-modal .modal-content {
                    width: 95% !important;
                ...
                #add-consultation-form {
                    display: block !important; /* Allow grid child to take full width */
                    width: 100% !important;
            ...
            <main class="container nav-space">
                            ...
                            <?php foreach ($this->consultations as $consultation) :
                                ...
                                <article class="consultation" id="consultation-<?php echo $consultation->getId(); ?>"
                                    ...
                                    <div class="consultation-header">
                                        ...
                                        <div class="header-right">
                                            ...
                                            <span class="date-badge <?php echo $isPast ? 'has-tooltip' : ''; ?>"
                                                ...
                                                <?php if ($this->isAdmin || (int) $consultation->getDoctorId() === (int) $this->currentUserId) : 
                                                    $docsJson = [];
                                                    foreach ($consultation->getDocuments() as $doc) {
                                                        $docsJson[] = [
                                                            'id' => (int) $doc->getId(),
                                                            'filename' => $doc->getFilename(),
                                                            'size' => $doc->getHumanSize()
                                                        ];
                                                    }
                                                ?>
                                                    <div class="action-buttons">
                                                        <button type="button" class="btn-icon edit-btn" title="Modifier"
                                                            ...
                                                            data-title="<?= $title ?>" 
                                                            data-note="<?= $note ?>"
                                                            data-documents='<?php echo json_encode($docsJson); ?>'>
                                    ...
                                    <div class="consultation-footer">
                                        <?php $docs = $consultation->getDocuments(); ?>
                                            ...
                                            <?php if (!empty($docs)) : ?>
                                                ...
                                                <div class="doc-list">
                                                    <?php foreach ($docs as $doc) : ?>
                                                        <div class="doc-item">
                                                            <a href="/?page=api_consultation_document&id=<?= (int) $doc->getId() ?>"
                                                               target="_blank" class="doc-link" title="Ouvrir le PDF">
                                                                ...
                                                                <span class="doc-filename"><?= htmlspecialchars($doc->getFilename(), ENT_QUOTES, 'UTF-8') ?></span>
                                                                <span class="doc-size"><?= htmlspecialchars($doc->getHumanSize()) ?></span>
                                                            </a>
                                                            <?php if ($canEdit) : ?>
                                                                <form method="POST" class="doc-delete-form">
                                                                    <input type="hidden" name="action" value="delete_document_pdf">
                                                                    <input type="hidden" name="id_document" value="<?= (int) $doc->getId() ?>">
                                                                    <button type="submit" class="doc-delete-btn" title="Supprimer ce document">
                ...
                <div id="add-consultation-modal" class="modal-backdrop hidden">
                    <div class="modal-content">
                        ...
                        <form id="add-consultation-form" method="POST" action="?page=medicalprocedure" enctype="multipart/form-data">
                            ...
                            <input type="hidden" name="id_patient" value="<?php echo $this->patientId; ?>">

                            <div class="form-grid">
                                <div class="form-group">
                                    <label for="consultation-doctor">Médecin</label>
                                    <?php if ($this->isAdmin) : ?>
                                        <select id="doctor-select" name="doctor_id" required>
                                            <option value="">Sélectionner un médecin</option>
                                            <?php foreach ($this->doctors as $doc) :
                                                $currId = $doc['id_user'] ?? 0;
                                    ...
                                    <?php else : ?>
                                        ...
                                        foreach ($this->doctors as $doc) {
                                            ...
                                            if ((int)$idUserVal === (int) $this->currentUserId) {
                                                $lName = $doc['last_name'] ?? '';
                                        ...
                                        ?>
                                        <input type="text" value="<?= htmlspecialchars($docName) ?>" disabled class="form-control-disabled">
                                        <input type="hidden" name="doctor_id" value="<?= $this->currentUserId ?>">
                                    <?php endif; ?>
                                </div>
                                ...
                                <div class="form-full-width">
                                    <div class="form-group">
                                        ...
                                        <textarea id="consultation-note" name="consultation_note" rows="3" placeholder="Détails de la consultation..."></textarea>
                                    </div>

                                    <div id="modal-document-section" class="form-group" style="margin-top: 12px; padding-top: 12px; border-top: 1px dashed var(--border-subtle);">
                                        <label style="display: flex; align-items: center; gap: 8px; color: var(--text-main); font-weight: 600; font-size: 0.9rem; margin-bottom: 8px;">
                                        ...
                                        <div id="modal-existing-documents" class="doc-list" style="display: none; margin-bottom: 8px;">
                                            <!-- Populated by JS -->
                                        </div>

                                        <div class="upload-container" style="background: rgba(var(--bg-canvas-rgb), 0.3); padding: 12px; border-radius: 10px; border: 1px dashed var(--border-subtle);">
                                            <input type="file" name="pdf_file[]" id="consultation-pdf" accept=".pdf" multiple style="font-size: 0.85rem; width: 100%;">
                                        </div>
                                    </div>
                                </div>
                            </div>
